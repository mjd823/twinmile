...
/**
 * Shared FMCSA Company Census prospecting core.
 *
 ...
 *
 * Used by BOTH:
 *   - app/api/cron/prospecting/route.ts (Sofia's daily 8AM CT run)
 *   - app/admin/api/ai-activity/route.ts ("outbound_prospecting" on-demand
 *     action behind the "Run Prospecting" button on the agent dashboard)
 *
 * Env (all optional, overridable per call via ProspectingOptions):
...
export interface CensusCarrier {
  dot_number: string;
  legal_name: string;
  dba_name?: string;
  phy_city?: string;
...
export interface ProspectingOptions {
  /** Max prospects to save this run. Default: FMCSA_MAX_RESULTS env or 30. */
  maxResults?: number;
  /** States to sample. Default: FMCSA_TARGET_STATES env or TX,LA,CA,GA,TN. */
  targetStates?: string[];
  /** Socrata app token. Default: SOCRATA_APP_TOKEN env. */
  appToken?: string;
}
...
async function queryCarriersByState(
  ...
  try {
    ...
    console.error(
      `[fmcsa-prospecting] Error querying ${state}:`,
...
/**
 * Run one REAL FMCSA Census prospecting pass: query target states, score,
 * dedupe by DOT number, and insert new prospects into outbound_prospects.
 * Returns honest counts — 0 found/saved is a valid result.
 *
 * Does NOT write agent_activity; callers log their own run record.
 */
export async function runFmcsaProspecting(
  ...
  const maxResults =
    options.maxResults ?? parseInt(process.env.FMCSA_MAX_RESULTS || "30", 10);
  const targetStates =
    options.targetStates ??
    (process.env.FMCSA_TARGET_STATES || "TX,LA,CA,GA,TN")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
