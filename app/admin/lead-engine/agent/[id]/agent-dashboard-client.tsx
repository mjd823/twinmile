...
const AGENT_WORKFLOW_STEPS: Record<string, Array<{ step: number; label: string; description: string; status: "pending" | "active" | "complete" }>> = {
  lead_generation: [
    ...
    { step: 5, label: "Route to Team", description: "Premium → CEO, Driver → HR, Freight → Sales, Nurture → Marketing", status: "pending" },
    { step: 6, label: "Log & Track", description: "Save to MongoDB with score, routing, and next action timestamp", status: "pending" },
  ],
  sales: [
    { step: 1, label: "Receive Lead", description: "Get qualified quote lead routed from Sofia (≥70 score)", status: "complete" },
  ...
  hr: [
    { step: 1, label: "Receive Application", description: "Get scored driver lead routed from Sofia (≥75)", status: "complete" },
...
export function AgentDashboardClient({
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      // Advance workflow step on success
      if (result.success && workflowStep < workflowSteps.length - 1) {
  ...
  const handleWorkflowAdvance = async () => {
    ...
    setWorkflowStep(w => w + 1);
    // Re-run the action for the next step
    setActionLoading(true);
    try {
      const result = await clientAIActivityEngine.handleDashboardAction(agentAction?.apiAction || "");
      setActionResult(result);
    } catch (error) {
      setActionResult({ success: false, message: String(error) });
    } finally {
      setActionLoading(false);
    }
  };
      ...
      <Tabs defaultValue="action" className="space-y-6">
            ...
            <CardContent>
              {recentActivity.length > 0 ? (
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Execute the primary action above to start.</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="performance" className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-3">
            <Card className="border-border/60 bg-green-500/5 border-green-500/30">
              <CardContent className="p-6 text-center">
                <BarChart3 className="h-10 w-10 text-green-400 mx-auto mb-2" />
            ...
            <Card className="border-border/60 bg-blue-500/5 border-blue-500/30">
              <CardContent className="p-6 text-center">
                <CheckCircle className="h-10 w-10 text-blue-400 mx-auto mb-2" />
            ...
            <Card className="border-border/60 bg-purple-500/5 border-purple-500/30">
              <CardContent className="p-6 text-center">
                <Target className="h-10 w-10 text-purple-400 mx-auto mb-2" />
          ...
          <Card className="border-border/60">
            ...
            <CardContent>
              <div className="space-y-3">
                {["outbound_prospecting", "find_customers", "hire_drivers", "schedule_deliveries", "check_revenue", "customer_support", "strategic_review", "send_marketing", "daily_ops", "weekly_review", "monthly_bi", "driver_engagement", "auto_onboarding"].map(actionType => {
                  const count = recentActivity.filter(a => a.type === actionType || a.action === actionType).length;
                  if (count === 0) return null;
                  return (
                    <div key={actionType} className="flex items-center gap-4">
                      <div className="w-36 text-xs text-muted-foreground font-medium truncate">{actionType.replace(/_/g, " ")}</div>
                      <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                        <div className="h-full bg-primary rounded-full" style={{ width: `${Math.min((count / Math.max(1, recentActivity.length)) * 100, 100)}%` }} />
                      </div>
