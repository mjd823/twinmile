...
const UpdateQuoteLeadSchema = z.object({
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const parsed = UpdateQuoteLeadSchema.safeParse(body);
  if (!parsed.success) {
  ...
  await db.collection("leads_quotes").updateOne({ _id: leadObjectId }, update as any);
